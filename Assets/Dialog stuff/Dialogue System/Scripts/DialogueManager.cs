...
namespace HeneGames.DialogueSystem
{
    public class DialogueManager : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (Input.GetKeyDown(DialogueUI.instance.actionInput) && dialogueTrigger != null && !dialogueIsOn || Input.GetKeyDown(DialogueUI.instance.actionInput2) && dialogueTrigger != null && !dialogueIsOn)
            {
        ...
        {
            if(Player.GetComponent<Movement2D>() == null)
            {
                Player.GetComponent<Movement>().DialogSystem = this;
            }
            else
            {
                Player.GetComponent<Movement2D>().DialogSystem = this;
            }
            FreezePlayer = true;
            //Start event
            if(dialogueTrigger != null)
